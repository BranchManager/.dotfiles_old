...
current_output=$(pactl list-sinks | sed -n '/\* index/,/index/p' | grep -e "device.description" |  cut -f2 -d\")
...
options=$( pactl list-sinks | grep "device.description" | cut -f2 -d\")
readarray -t lines < <(pactl list-sinks | grep "device.description" | cut -f2 -d\")
...
readarray -t actnames< <(pactl list-sinks | grep "name:" | cut -f2 -d\ | tr -d '<>,')
...
pactl set-default-sink "${actnames[$my_selection]}"
